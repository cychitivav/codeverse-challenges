 ...
 * case contains an integer N (denoting the length of the string) and a string S.
 * 
 * Output Format:
...
public class MagicalWord {
    ...
    public static void main(String[] args) {
        ...
        char prime[] = {67, 71, 73, 79, 83, 89, 97, 101, 103, 107, 109, 113, 113};
            ...
            for (int i = 0; i < N; i++) {
                for (int k = 0; k < prime.length; k++) {
                    if (S[i] > 113) {
                        S[i] = 113;
                        break;
                    } else if (prime[k + 1] >= S[i]) {
                        if (S[i] - prime[k] <= prime[k + 1] - S[i]) {
                            S[i] = (char) (prime[k]);
                        } else {
                            S[i] = (char) (prime[k + 1]);
                        }
